...
import qualified Data.ByteString             as BS (empty, length, null, take, takeWhile)
import           Data.ByteString.Char8       as C (replicate)
import           Data.Serialize              (Serialize, get, put)
import           Data.Serialize.Get          (Get, getByteString, getWord16be,
                                              getWord16le, getWord32be, getWord32host,
                                              getWord32le, getWord64le, getWord8, isEmpty)
import           Data.Serialize.Put          (Put, putByteString, putWord16be,
                                              putWord16le, putWord32be, putWord32host,
...
reject cmd code reason = Reject cmd code (VarString reason) BS.empty

instance Serialize Reject where
    get =
        get >>=
        \(VarString bs) ->
             case stringToCommand bs of
                 Just cmd -> Reject cmd <$> get <*> get <*> maybeData
                 _ ->
                     fail $
                     unwords ["Reason get: Invalid message command", cs bs]
      where
        maybeData =
            isEmpty >>=
            \done ->
...
instance NFData MessageCommand where
    rnf x = seq x ()
...
instance Serialize MessageCommand where
    get = go =<< getByteString 12
      where
        go bs =
            case stringToCommand $ unpackCommand bs of
                Just cmd -> return cmd
...
packCommand s = BS.take 12 $ s `mappend` C.replicate 12 '\NUL'
