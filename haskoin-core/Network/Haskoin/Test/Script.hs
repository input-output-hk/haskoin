...
instance Arbitrary ArbitraryScriptOp where
    arbitrary =
        ...
        -- Pushing Data
            [ arbitrary >>=
              \(ArbitraryNotNullByteString bs) -> return $ opPushData bs
            , return OP_0
            , return OP_1NEGATE
            , return OP_RESERVED
            , return OP_1
...
instance Arbitrary ArbitraryPKInput where
    arbitrary =
        ArbitraryPKInput . RegularInput . SpendPK <$>
...
instance Arbitrary ArbitraryPKHashCInput where
    arbitrary = do
        ...
        return $
            ArbitraryPKHashCInput $ RegularInput $ SpendPKHash sig $ toPubKeyG key
