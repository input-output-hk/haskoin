...
dumpOp (OP_PUSHDATA payload optype) =
    mconcat ["OP_PUSHDATA(", cs (show optype), ")", " 0x", encodeHex payload]
...
evalStackOperation :: StackOperation a
                   -> ProgramData
                   -> FlagSet
                   -> Either EvalError a
evalStackOperation m s f =
    runIdentity . runExceptT $ evalStateT (runReaderT m f) s

evalProgram
    :: Program a -- ^ ProgramData monad
...
popInt =
    minimalStackValEnforcer >> decodeInt <$> popStack >>=
    \case
...
checkPushData (OP_PUSHDATA v _)
    | BS.length v > fromIntegral maxScriptElementSize =
...
pickStack remove n = do
    st <- getStack
    when (n < 0) $ programError "pickStack: n < 0"
    when (n > length st - 1) $ programError "pickStack: n > size"
    let v = st !! n
...
dropHashOpsSeparatedCode =
    modify $
    \p ->
         let tryDrop = dropWhile (/= OP_CODESEPARATOR) $ hashOps p
         in case tryDrop
...
findAndDelete (s:ss) ops =
    let pushOp = opPushData . opToSv $ s
    in findAndDelete ss $ filter (/= pushOp) ops

checkMultiSig
    :: SigCheck -- ^ Signature checking function
    -> [StackValue] -- ^ PubKeys
...
orderedSatisfy :: (a -> b -> Bool) -> [a] -> [b] -> Bool
orderedSatisfy _ [] _ = True
orderedSatisfy _ (_:_) [] = False
orderedSatisfy f x@(a:as) y@(b:bs)
...
tStack4
    :: (StackValue -> StackValue -> StackValue -> StackValue -> Stack)
    -> StackOperation ()
tStack4 f = f <$> popStack <*> popStack <*> popStack <*> popStack >>= prependStack

tStack6
    :: (StackValue -> StackValue -> StackValue -> StackValue -> StackValue -> StackValue -> Stack)
    -> StackOperation ()
tStack6 f =
    f <$> popStack <*> popStack <*> popStack <*> popStack <*> popStack <*> popStack >>=
    prependStack
...
incrementOpCount i
    | i > maxOpcodes = programError "reached opcode limit"
    | otherwise =
        modify $
        \p ->
             p
...
eval OP_BOOLAND = (&&) <$> ((0 /=) <$> popInt) <*> ((0 /=) <$> popInt) >>= pushBool
...
eval OP_HASH160 =
    tStack1 $ return . bsToSv . getHash160 . hash160 . getHash256 . hash256 . opToSv
eval OP_HASH256 = tStack1 $ return . bsToSv . getHash256 . doubleHash256 . opToSv
...
eval OP_CHECKMULTISIG = do
    nPubKeys <- fromIntegral <$> popInt
...
execScript scriptSig scriptPubKey sigCheckFcn flags =
    let sigOps = scriptOps scriptSig
        ...
        checkSig
            | isPayToScriptHash pubKeyOps flags = checkPushOnly sigOps
    ...
    in do s <- evalProgram redeemEval [] initData flags
          p <-
              evalProgram
                  ...
                  flags
          if (not . null $ s) &&
             (isPayToScriptHash pubKeyOps flags) && (checkStack . runStack $ p)
              then evalProgram
                       (evalP2SH s)
                       []
                       initData
...
verifySigWithType tx i outOps txSig pubKey =
    let outScript = Script outOps
        h = txSigHash tx outScript i (sigHashType txSig)
