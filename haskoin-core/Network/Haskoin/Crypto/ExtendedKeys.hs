...
instance Show XPrvKey where
    showsPrec d k =
        showParen (d > 10) $ showString "XPrvKey " . shows (xPrvExport k)

-- TODO: Test
instance Read XPrvKey where
    readPrec =
        parens $
        do Read.Ident "XPrvKey" <- lexP
           Read.String str <- lexP
           maybe pfail return $ xPrvImport $ cs str

-- TODO: Test
instance IsString XPrvKey where
    fromString = fromMaybe e . xPrvImport . cs
      where
...
instance Show XPubKey where
    showsPrec d k =
        showParen (d > 10) $ showString "XPubKey " . shows (xPubExport k)

-- TODO: Test
instance Read XPubKey where
    readPrec =
        parens $
        do Read.Ident "XPubKey" <- lexP
           Read.String str <- lexP
           maybe pfail return $ xPubImport $ cs str

-- TODO: Test
instance IsString XPubKey where
    fromString = fromMaybe e . xPubImport . cs
      where
...
xPrvFP = either (const err) id . decode . BS.take 4 . getHash160 . xPrvID
  where
    err = error "Could not decode xPrvFP"

-- | Computes the key fingerprint of an extended public key.
xPubFP :: XPubKey -> Word32
xPubFP = either (const err) id . decode . BS.take 4 . getHash160 . xPubID
  where
...
deriveMSAddr keys m i = (scriptAddr rdm, rdm)
  where
...
instance Show DerivPath where
    showsPrec d p =
        showParen (d > 10) $ showString "DerivPath " . shows (pathToStr p)

-- TODO: Test
instance Show HardPath where
    showsPrec d p =
        showParen (d > 10) $ showString "HardPath " . shows (pathToStr p)

-- TODO: Test
instance Show SoftPath where
    showsPrec d p =
        showParen (d > 10) $ showString "SoftPath " . shows (pathToStr p)

-- TODO: Test
instance Read DerivPath where
    readPrec =
        parens $
        do Read.Ident "DerivPath" <- lexP
           Read.String str <- lexP
