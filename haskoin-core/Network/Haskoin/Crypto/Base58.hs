...
instance Serialize Address where
    get = do
      ...
      where
        f x a
            | x == addrPrefix = return (PubKeyAddress a)
...
instance Show Address where
    showsPrec d a =
        showParen (d > 10) $ showString "Address " . shows (addrToBase58 a)

-- TODO: Test
instance Read Address where
    readPrec =
        parens $
        do Read.Ident "Address" <- lexP
           Read.String str <- lexP
           maybe pfail return $ base58ToAddr $ cs str

-- TODO: Test
instance IsString Address where
    fromString = fromMaybe e . base58ToAddr . cs
      where
...
instance FromJSON Address where
    parseJSON = withText "Address" $ maybe mzero return . base58ToAddr . cs
