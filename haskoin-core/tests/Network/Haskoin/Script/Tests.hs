...
import qualified Test.HUnit                           as HUnit (assertBool, assertFailure)
...
import qualified Data.ByteString                      as BS (empty, head, length, pack,
                                                             singleton, tail, unpack)
...
import           Network.Haskoin.Internals            (Flag, decodeBool, decodeInt,
                                                       dumpStack, encodeBool, encodeInt,
                                                       execScript, runStack)
...
testEncodeSH32 (ArbitrarySigHash sh) =
    BS.length bs == 4 &&
    BS.head bs == (BS.head $ encode sh) && BS.tail bs == BS.pack [0, 0, 0]
  where
...
testSigHashOne (ArbitraryTx tx) (ArbitraryScript s) acp =
    not (null $ txIn tx) ==>
...
testFile groupLabel path expected =
    ...
    do dat <- C.readFile path
       case (A.decode dat) :: Maybe [[String]] of
           ...
           Just testDefs ->
               return $
               testGroup groupLabel $ map parseTest $ filterPureComments testDefs
  where
...
buildCreditTx scriptPubKey = createTx 1 [txI] [txO] 0
  where
    txO =
        TxOut
        { outValue = 0
...
buildSpendTx
    ...
    -> Tx -- Creditting Tx
    -> Tx
buildSpendTx scriptSig creditTx = createTx 1 [txI] [txO] 0
  where
    txI =
        TxIn
        { prevOutput =
            OutPoint
            { outPointHash = txHash creditTx
            , outPointIndex = 0
            }
