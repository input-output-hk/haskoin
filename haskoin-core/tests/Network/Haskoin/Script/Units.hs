...
module Network.Haskoin.Script.Units
  ( tests
  ) where
...
sigDecodeMap (_, i) = testCase ("Signature " ++ show i) func
  where
...
testSigDecode str =
    ...
    in assertBool (unwords ["Decode failed:", fromLeft eitherSig]) $ isRight eitherSig
