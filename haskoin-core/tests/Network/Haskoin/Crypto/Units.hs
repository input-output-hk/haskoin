...
module Network.Haskoin.Crypto.Units
  ( tests
  ) where
    ...
    , testGroup "bitcoind /src/test/key_tests.cpp" $
      ...
      (map
           (\x ->
                 (testCase
                      ("Check sig: " ++ (show x))
                      (checkSignatures $ doubleHash256 x)))
           sigMsg)
    , testGroup
...
checkInvalidKey = assertBool "Bad key" $ isNothing $ fromWif strAddressBad
