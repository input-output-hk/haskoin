module Network.Haskoin.Cereal.Tests
  ( tests
  ) where
...
tests =
    [ testGroup
          "Binary encoding and decoding of utility types"
          [testProperty "ByteString" $ \(ArbitraryByteString x) -> metaBinary x]
    , testGroup
          "Binary encoding and decoding of hash types"
    ...
    , testGroup
          "Binary encoding and decoding of crypto types"
          [ testProperty "Signature" $
            \(ArbitrarySignature _ _ x) -> metaBinary x
    ...
    , testGroup
          "Binary encoding and decoding of protocol types"
          [ testProperty "VarInt" $ \(ArbitraryVarInt x) -> metaBinary x
          , testProperty "VarString" $ \(ArbitraryVarString x) -> metaBinary x
          , testProperty "NetworkAddress" $
            \(ArbitraryNetworkAddress x) -> metaBinary x
          ...
          , testProperty "MessageCommand" $
            \(ArbitraryMessageCommand x) -> metaBinary x
          , testProperty "MessageHeader" $
            \(ArbitraryMessageHeader x) -> metaBinary x
          , testProperty "Message" $ \(ArbitraryMessage x) -> metaBinary x
          ]
    , testGroup
          "Binary encoding and decoding of script types"
    ...
    , testGroup
          "Binary encoding and decoding of transaction types"
    ...
    , testGroup
          "Binary encoding and decoding of block types"
          [ testProperty "Block" $ \(ArbitraryBlock x) -> metaBinary x
          , testProperty "BlockHeader" $
            \(ArbitraryBlockHeader x) -> metaBinary x
          ...
          , testProperty "MerkleBlock" $
            \(ArbitraryMerkleBlock x) -> metaBinary x
          ]
    , testGroup
          "Binary encoding and decoding of bloom types"
          [ testProperty "BloomFlags" $ \(ArbitraryBloomFlags x) -> metaBinary x
          , testProperty "BloomFilter" $
            \(ArbitraryBloomFilter _ _ x) -> metaBinary x
