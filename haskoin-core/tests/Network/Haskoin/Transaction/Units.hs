...
module Network.Haskoin.Transaction.Units
  ( tests
  , satoshiCoreTxTests
  ) where
...
import           Data.Serialize                 (decode, encode, getWord32le, putWord32le,
                                                 runGet, runPut)
...
tests :: [Test]
tests =
    [ testGroup "Computing TxID from Tx" (map mapTxIDVec $ zip txIDVec [0 ..])
    , testGroup
          "Build PKHash Transaction (generated from bitcoind)"
          (map mapPKHashVec $ zip pkHashVec [0 ..])
    , testGroup
          "Verify transaction (bitcoind /test/data/tx_valid.json)"
...
mapPKHashVec
    :: (([(ByteString, Word32)], [(ByteString, Word64)], ByteString), Int)
    -> Test.Framework.Test
mapPKHashVec (v, i) = testCase name $ runPKHashVec v
  where
    name = "Build PKHash Tx " ++ (show i)

runPKHashVec :: ([(ByteString, Word32)], [(ByteString, Word64)], ByteString)
             -> Assertion
runPKHashVec (xs, ys, res) =
    assertBool "Build PKHash Tx" $ (encodeHex $ encode tx) == res
  where
    tx = fromRight $ buildAddrTx (map f xs) ys
    f (tid, ix) = OutPoint (fromJust $ hexToTxHash tid) ix

mapVerifyVec :: (SatoshiCoreTxTest, Int) -> Test.Framework.Test
mapVerifyVec (v@(SatoshiCoreTxTest d _ _), i) = testCase name $ runVerifyVec v i
  where
    name = "Verify Tx " ++ (show i) ++ ", about: " ++ d
  ...
  where
    ...
    f (SatoshiCoreTxTestInput bsOutputHash bsOutputIndex bsOutputScriptPubKey) =
        let s :: ScriptOutput
            s =
                either
                    (const $ error $ "could not decode: " ++ convertString bsOutputScriptPubKey)
                    id .
...
data SatoshiCoreTxTest =
    SatoshiCoreTxTest String
                      [SatoshiCoreTxTestInput]
                      ByteString
    deriving (Read, Show)

data SatoshiCoreTxTestInput =
    SatoshiCoreTxTestInput ByteString
                           ByteString
                           ByteString
    deriving (Read, Show)
...
verifyVec =
    let toSatCoreTest (is, sertx, description) =
            SatoshiCoreTxTest description (map toInputs is) sertx
    ...
    encodeSatoshiCoreScriptPiece s =
        case (readMay ("OP_" ++ s) :: Maybe ScriptOp) of
            ...
            Nothing ->
                case (take 2 s) of
                    "OP" ->
                        encodeHex . encode . (read :: String -> ScriptOp) $ s
                    ...
                    _ ->
                        case (readMay s :: Maybe Int) -- can we get rid of this case now?
                            ...
                            Nothing ->
                                error $ "encodeSatoshiCoreScriptPubKey: " ++ s
...
satoshiCoreTxVec = do
    ...
    let testsAndComments =
            maybe (error $ "satoshiCoreTxVec, couldn't decode json") id .
            Aeson.decode $
            tx_validBS
    return $
