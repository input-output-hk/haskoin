...
getSigningKeys name = do
    ...
    case kM of
        ...
        Nothing -> do
            keyOrMnemonic <-
                liftIO . Haskeline.runInputT Haskeline.defaultSettings $
    ...
    handleResponse resE $
        \WT.JsonAddr {..} ->
             ...
             \ts -> do
                 let xs =
                         map
                             (liftIO . putStr . printTx (Just jsonAddrAddress))
                             ts
                     xs' =
                         if r
                             then xs
            ...
            handleResponse resE $
                \(WT.OfflineTxData tx dat) -> do
                    ...
                    liftIO $
                        ...
                        unwords
                            [ "CoinData:"
                            , cs $ encodeHex $ cs $ Aeson.encode dat
                            ]
        _ -> error "Could not parse txid"
  where
...
cmdSignOffline name txStr datStr =
    case (txM, datM) of
        (Just tx, Just dat) -> do
            masterM <- getSigningKeys name
...
cmdVersion =
    liftIO $
    do putStrLn $ unwords ["network   :", cs networkName]
...
cmdKeyPair = do
    ...
    liftIO $
        do B8.putStrLn $ B8.unwords ["public :", rvalue pub]
...
handleNotif fmt (Right notif) =
    case fmt of
        ...
        OutputYAML -> do
            ...
            putStrLn "..."
        OutputNormal -> putStrLn $ printNotif notif
...
sendZmq req = do
    ...
    when (configVerbose cfg) $
        liftIO $ B8.hPutStrLn stderr $ "Outgoing JSON: " `mappend` msg
...
setupAuth cfg sock = do
    ...
    forM_ clientKeyM $
        \clientKey -> do
            let serverKeyPub =
                    fromMaybe (error "Server public key not provided") serverKeyPubM
                clientKeyPub =
                    fromMaybe (error "Client public key not provided") clientKeyPubM
...
encodeOutPointJSON (OutPoint h i) =
    object ["txid" .= (cs $ txHashToHex h :: Text), "pos" .= i]

encodeScriptJSON :: Script -> Value
encodeScriptJSON (Script ops) = toJSON $ map f ops
  where
    f (OP_PUSHDATA bs _) =
        String $ pack $ unwords ["OP_PUSHDATA", cs $ encodeHex bs]
    f x = String $ pack $ show x

encodeScriptInputJSON :: ScriptInput -> Value
encodeScriptInputJSON si =
    case si of
        RegularInput (SpendPK s) ->
            object ["spendpubkey" .= object ["sig" .= encodeSigJSON s]]
        RegularInput (SpendPKHash s p) ->
            object
                [ "spendpubkeyhash" .=
                  object
                      [ "sig" .= encodeSigJSON s
        ...
        RegularInput (SpendMulSig sigs) ->
            object ["spendmulsig" .= object ["sigs" .= map encodeSigJSON sigs]]
        ScriptHashInput s r ->
            object
                [ "spendscripthash" .=
                  object
...
encodeScriptOutputJSON so =
    case so of
        PayPK p ->
            object
                [ "pay2pubkey" .=
                  object ["pubkey" .= (cs $ encodeHex (encode p) :: Text)]
                ]
        PayPKHash a ->
            object
                [ "pay2pubkeyhash" .=
            ...
            object
                [ "pay2mulsig" .=
                  object
                      [ "required-keys" .= r
                      , "pubkeys" .=
                        (map (cs . encodeHex . encode) ks :: [Text])
                      ]
                ]
        PayScriptHash a ->
            object
...
encodeSigJSON ts@(TxSignature _ sh) =
    object
...
encodeSigHashJSON sh =
    case sh of
        SigAll acp -> object ["type" .= String "SigAll", "acp" .= acp]
        SigNone acp -> object ["type" .= String "SigNone", "acp" .= acp]
        SigSingle acp -> object ["type" .= String "SigSingle", "acp" .= acp]
        SigUnknown acp v ->
...
printNodeStatus verbose STM.NodeStatus {..} =
    ...
    , verbose ] ++
    [ "Merkle Peer       : " ++ show m
    | m <- maybeToList nodeStatusMerklePeer
    , verbose ] ++
    ...
    | r <- maybeToList nodeStatusRescan
    , verbose ] ++
    [ "Mempool size      : " ++ show nodeStatusMempool
    | verbose ] ++
...
printPeerStatus verbose STM.PeerStatus {..} =
    ...
    [ "  Protocol : " ++ show p
    | p <- maybeToList peerStatusProtocol ] ++
    [ "  UserAgent: " ++ ua
    | ua <- maybeToList peerStatusUserAgent ] ++
    [ "  Avg Ping : " ++ p
    | p <- maybeToList peerStatusPing ] ++
    [ "  DoS Score: " ++ show d
    | d <- maybeToList peerStatusDoSScore ] ++
    [ "  Merkles  : " ++ show peerStatusHaveMerkles
    | verbose ] ++
    ...
    | verbose ] ++
    [ "  Reconnect: " ++ show t
    | t <- maybeToList peerStatusReconnectTimer
    , verbose ] ++
    [ "  Logs     : "
    | verbose ] ++
    [ "    - " ++ msg
    | msg <- fromMaybe [] peerStatusLog
    , verbose ]
