...
module Network.Haskoin.Wallet.Units
  ( tests
  ) where
...
import           Database.Persist.Sqlite          (SqlPersistT, runMigrationSilent,
                                                   runSqlite)
...
tests =
    [ testGroup
          ...
          , testCase "Creating two accounts with the same data should fail" $
            assertException (WalletException "Account already exists") $
            do _ <-
                   newAccount
                       NewAccount
                       { newAccountName = "main"
          ...
          , testCase
                "Calling addAccountKeys on a non-multisig account should fail" $
            assertException (WalletException "The account is already complete") $
            do res <-
                   newAccount
                       NewAccount
    ...
    , testGroup
          ...
          [ testCase "Decreasing the address gap should fail" $
            assertException
                (WalletException "The gap of an account can only be increased") $
            do res <-
                   newAccount
                       NewAccount
          ...
          , testCase "Setting a label on a hidden address key should fail" $
            assertException (WalletException "Invalid address index 10") $
            do res <-
                   newAccount
                       NewAccount
          ...
          , testCase "Setting a label on an invalid address key should fail" $
            assertException (WalletException "Invalid address index 20") $
            do res <-
                   newAccount
                       NewAccount
          ...
          , testCase
                "Requesting an address prvkey on a read-only account should fail" $
            assertException (WalletException "Could not get private key") $
            do res <-
                   newAccount
                       NewAccount
                       { newAccountName = "label-invalid"
...
assertException ex action =
    handleJust matchEx (const $ return ()) $
    do runUnit action
...
runUnit action = do
    _ <-
        runSqlite ":memory:" $
        do _ <- runMigrationSilent migrateWallet
           initWallet 0.0001
           action
    return ()
...
fakeNode
    ...
    -> Word32 -- ^ Nonce
    -> NodeBlock
fakeNode parent tids chain nonce = nodeBlock parent chain header
  where
    header =
        createBlockHeader
...
fakeTx xs ys = createTx 1 txi txo 0
  where
...
testDerivations = do
    ...
    unusedAddresses accE AddressExternal (ListRequest 0 0 False) >>=
        liftIO .
        assertEqual
            "Generated external addresses do not match"
    ...
    unusedAddresses accE AddressInternal (ListRequest 0 0 False) >>=
        liftIO .
        assertEqual
            "Generated internal addresses do not match"
...
testBalances = do
    ...
    let tx1 =
            fakeTx
                [(txHash fundingTx, 0), (txHash fundingTx, 1)]
    ...
    assertAddress
        ...
        AddressExternal
        [(1, BalanceInfo 20000000 20000000 1 1)]
    assertAddress accE 3 0 AddressExternal [(0, BalanceInfo 0 0 0 0)]
    assertAddress accE 3 1 AddressExternal [(1, BalanceInfo 0 0 0 0)]
    -- Reorg on tx2
    let block2' = fakeNode block1 [] 1 22
        block3' = fakeNode block2 [txHash tx2] 1 33
    importMerkles
        (ChainReorg block1 [block2] [block2', block3'])
        [[], [txHash tx2]]
        Nothing
    ...
    assertAddress
        ...
        AddressExternal
        [(0, BalanceInfo 10000000 10000000 1 1)]
    assertAddress accE 3 1 AddressExternal [(1, BalanceInfo 20000000 0 1 0)]
    assertAddress accE 0 0 AddressInternal [(0, BalanceInfo 5000000 0 1 0)]
    assertAddress accE 1 0 AddressInternal [(0, BalanceInfo 5000000 0 1 0)]
    assertAddress accE 2 0 AddressInternal [(0, BalanceInfo 0 0 0 0)]
    ...
    accountBalance ai 3 False >>=
        liftIO . assertEqual "Balance is not 20000000" 20000000
    accountBalance ai 4 False >>= liftIO . assertEqual "Balance is not 0" 0
    assertAddress
        accE
        0
        0
    ...
    importMerkles
        (ChainReorg block1 [block2', block3'] [block2, block3, block4])
        [[txHash tx1], [], []]
        Nothing
    assertBalance ai 0 0
    assertBalance ai 1 0
    ...
    assertAddress
        ...
        AddressExternal
        [(1, BalanceInfo 20000000 20000000 1 1)]
    assertAddress accE 5 0 AddressExternal [(0, BalanceInfo 0 0 0 0)]
    assertAddress accE 5 1 AddressExternal [(1, BalanceInfo 0 0 0 0)]
    assertAddress accE 0 0 AddressInternal [(0, BalanceInfo 0 0 0 0)]
...
testConflictBalances = do
    ...
    assertAddress
        ...
        AddressExternal
        [(0, BalanceInfo 10000000 10000000 1 1)]
    assertAddress accE 0 0 AddressInternal [(0, BalanceInfo 4000000 0 1 0)]
    -- Let's confirm these two transactions
    let block1 = fakeNode genesisBlock [txHash tx1] 0 1
        block2 = fakeNode block1 [txHash tx2] 0 2
    importMerkles (BestChain [block1, block2]) [[txHash tx1], [txHash tx2]] Nothing
    assertBalance ai 0 4000000
    ...
    assertAddress
        ...
        AddressExternal
        [(0, BalanceInfo 10000000 10000000 1 1)]
    assertAddress accE 1 0 AddressInternal [(0, BalanceInfo 4000000 0 1 0)]
    assertAddress
        accE
        2
        0
    ...
    assertBalance ai 2 20000000
    assertBalance ai 3 0
    assertAddress accE 0 0 AddressExternal [(0, BalanceInfo 20000000 0 1 0)]
    assertAddress accE 0 0 AddressInternal [(0, BalanceInfo 0 0 0 0)]
    assertAddress accE 2 0 AddressExternal [(0, BalanceInfo 20000000 0 1 0)]
    assertAddress accE 3 0 AddressExternal [(0, BalanceInfo 0 0 0 0)]
    ...
    assertAddress
        ...
        AddressExternal
        [(0, BalanceInfo 10000000 10000000 1 1)]
    assertAddress accE 3 0 AddressInternal [(0, BalanceInfo 4000000 0 1 0)]
    assertAddress
        accE
        4
        0
        AddressExternal
        [(0, BalanceInfo 10000000 10000000 1 1)]
    assertAddress accE 4 0 AddressInternal [(0, BalanceInfo 0 0 0 0)]
    assertAddress accE 5 0 AddressExternal [(0, BalanceInfo 0 0 0 0)]
...
testOffline = do
    ...
    assertBalance ai 1 0
    assertAddress accE 0 0 AddressExternal [(0, BalanceInfo 0 0 0 0)]
    assertAddressOffline
        accE
        0
        0
    ...
    assertAddressOffline accE 0 0 AddressInternal [(0, BalanceInfo 0 0 0 0)]
...
testKillOffline = do
    ...
    assertAddress accE 0 0 AddressInternal [(0, BalanceInfo 3000000 0 1 0)]
    assertAddressOffline
        accE
        0
        0
    ...
    let tx1 =
        ...
        tx4 =
            fakeTx
                [(tid1, 4)]
                [("1DLW4wieCwUPMh6ThVwT2bKqSzkjeb8wUe", 20000000)]
    assertException (WalletException "Could not import offline transaction") $
        do _ <-
               newAccount
                   NewAccount
                   { newAccountName = "acc1"
...
testImportMultisig = do
    ...
    let fundingTx =
            createTx
                ...
                [ TxOut 10000000 $
                  encodeOutputBS $
                  PayScriptHash $
                  fromJust $ base58ToAddr "32RexHZdsMoV8yzL1pQyFhYY6XeUNcWP78"
                ]
                0
    ...
    tx1 <-
        ...
        createWalletTx
            ...
            Nothing
            Nothing
            [ (fromJust $ base58ToAddr "3BYWaQHz6AVXx7wXmCka4846tRfa1ccWvh", 5000000)
            ]
            10000
            0
            False
            True
    liftIO $
        assertEqual "Confidence is not offline" TxOffline $ walletTxConfidence tx1
    ...
    liftIO $
        assertEqual "Txid do not match" (walletTxHash tx1) (walletTxHash tx2)
    liftIO $
        assertEqual "Confidence is not offline" TxOffline $ walletTxConfidence tx2
    ...
    liftIO $
        assertEqual "Confidence is not pending" TxPending $ walletTxConfidence tx3
    spendableCoins ai2 0 (const . const []) >>=
        liftIO .
        assertEqual "Wrong txhash in coins" [walletTxHash tx3, walletTxHash tx3] .
        map (walletCoinHash . entityVal . inCoinDataCoin)
    txs Nothing ai2 (ListRequest 0 10 False) >>=
        liftIO .
        assertEqual
    ...
    assertBalanceOffline ai2 0 9990000
    tx4 <-
        fmap (entityVal . fromJust) $ getBy $ UniqueAccTx ai1 $ walletTxHash tx3
    liftIO $
        assertEqual "Confidence is not pending" TxPending $ walletTxConfidence tx4
    spendableCoins ai1 0 (const . const []) >>=
        liftIO .
        assertEqual "Wrong txhash in coins" [walletTxHash tx3, walletTxHash tx3] .
        map (walletCoinHash . entityVal . inCoinDataCoin)
    ...
    liftIO $
        assertEqual "Confidence is not pending" TxPending $ walletTxConfidence tx5
    spendableCoins ai1 0 (const . const []) >>=
        liftIO .
        assertEqual "Wrong txhash in coins" [walletTxHash tx5, walletTxHash tx5] .
        map (walletCoinHash . entityVal . inCoinDataCoin)
    txs Nothing ai1 (ListRequest 0 10 False) >>=
        liftIO .
        assertEqual
...
testNotification = do
    ...
    liftIO $
        case b1NM of
            Just (NotifBlock JsonBlock {..}) ->
                assertEqual "Block hash does not match" (nodeHash block1) jsonBlockHash
            _ -> assertFailure "Block notification not the right type"
    tx1NM' <- liftIO $ atomically $ readTBMChan notifChan
    liftIO $
        case tx1NM' of
            Just (NotifTx JsonTx {..}) ->
                assertEqual "Transaction list does not match" (txHash tx1) jsonTxHash
            _ -> assertFailure "Transaction notification not the right type"
    b2NM <- liftIO $ atomically $ readTBMChan notifChan
    liftIO $
        case b2NM of
            Just (NotifBlock JsonBlock {..}) ->
                assertEqual "Block hash does not match" (nodeHash block2) jsonBlockHash
            _ -> assertFailure "Block notification not the right type"
    tx2NM' <- liftIO $ atomically $ readTBMChan notifChan
    liftIO $
        case tx2NM' of
            Just (NotifTx JsonTx {..}) ->
                assertEqual "Transaction list does not match" (txHash tx2) jsonTxHash
            _ -> assertFailure "Transaction notification not the right type"
    ...
    liftIO $
        case tx3NM2 of
            Just (NotifTx JsonTx {..}) ->
                assertEqual "Transaction does not match" (txHash tx2) jsonTxHash
            _ -> assertFailure "Transaction notification not the right type"
    tx3NM3 <- liftIO $ atomically $ readTBMChan notifChan
    liftIO $
        case tx3NM3 of
            Just (NotifTx JsonTx {..}) ->
                assertEqual "Transaction does not match" (txHash tx3) jsonTxHash
            _ -> assertFailure "Transaction notification not the right type"
...
testTx :: ([WalletTx], [WalletAddr]) -> ([(AccountId, TxConfidence)], Int)
