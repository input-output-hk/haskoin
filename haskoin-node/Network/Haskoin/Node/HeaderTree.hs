...
getVerifyParams
    :: MonadIO m
    => BlockHeader
    -> EitherT String (SqlPersistT m) (NodeBlock, [Timestamp], Timestamp, Word32, Maybe Word32)
getVerifyParams bh = do
    ...
    diffBlockM <-
        lift $
        getBlockByHeight parent $ nodeBlockHeight parent `div` diffInterval * diffInterval
    diffTime <-
        maybe
            (left "Could not get difficulty change block")
...
findMinWork
    :: MonadIO m
    => NodeBlock -> SqlPersistT m MinWork
findMinWork bn
    | isMinWork bn = return $ blockBits $ nodeHeader bn
    | otherwise =
        getParentBlock bn >>= maybe (return $ blockBits $ nodeHeader bn) findMinWork
...
splitKnown
    :: MonadIO m
    => [BlockHeader] -> SqlPersistT m ([NodeBlock], [BlockHeader])
splitKnown hs = do
    (kno, unk) <- foldM f ([], []) hs
...
connectHeader best bh ts =
    runEitherT $
    do (kno, _) <- lift $ splitKnown [bh]
       case kno of
           [] -> do
...
connectHeaders best bhs ts =
    runEitherT $
    do unless (validChain bhs) $ left "Block headers do not form a valid chain"
       ...
       case unk of
           ...
           (bh:_) -> do
               ...
               chain <- lift $ getChain parent
               nodes <-
                   (`evalStateT` (parent, diffTime, medians, minWork)) $
                   forM unk $
                   \b -> do
                       (p, d, ms, mw) <- get
                       ...
                       let bn = nodeBlock p chain b
                           d' =
                               if nodeBlockHeight bn `mod` diffInterval == 0
...
blockLocator
    :: MonadIO m
    => NodeBlock -> SqlPersistT m BlockLocator
...
verifyBlockHeader par mts dt cp mw ts bh = do
    ...
    when
        (networkName == "prodnet" &&
         blockVersion bh == 1 && nodeBlockHeight par + 1 >= 227836) $
...
nodeBlock parent chain bh =
  ...
  where
    newWork =
        nodeBlockWork parent +
        fromIntegral (headerWork bh `div` headerWork genesisHeader)
...
getParentBlock
    :: MonadIO m
    => NodeBlock -> SqlPersistT m (Maybe NodeBlock)
getParentBlock node
...
getChildBlocks
    :: MonadIO m
    => BlockHash -> SqlPersistT m [NodeBlock]
getChildBlocks h = do
...
lastSeenCheckpoint = fmap listToMaybe $ getBlocksByHash $ map snd $ reverse checkpointList
...
nextWorkRequired :: NodeBlock -> Timestamp -> MinWork -> BlockHeader -> Word32
...
headerWork bh = fromIntegral $ largestHash `div` (target + 1)
  where
    target = decodeCompact (blockBits bh)
...
{- Persistent backend -}
chainPathQuery
    :: forall (expr :: * -> *) (query :: * -> *) backend.
       Esqueleto query expr backend
    => expr (Entity NodeBlock) -> [NodeBlock] -> expr (Value Bool)
chainPathQuery _ [] = error "Monsters, monsters everywhere"
chainPathQuery t [NodeBlock {..}] =
    t ^. NodeBlockHeight <=. val nodeBlockHeight &&. t ^. NodeBlockChain ==.
    val nodeBlockChain
chainPathQuery t (n1:bs@(n2:_)) =
    chainPathQuery t bs ||.
    (t ^. NodeBlockHeight <=. val (nodeBlockHeight n1) &&. t ^. NodeBlockHeight >.
     val (nodeBlockHeight n2) &&.
     t ^.
     NodeBlockChain ==.
     val (nodeBlockChain n1))

getHeads
    :: MonadIO m
    => SqlPersistT m [NodeBlock]
getHeads =
    fmap (map (entityVal . snd)) $
    select $
    from $
    \t -> do
...
getChain parent = do
    ...
    if maxHeight == nodeBlockHeight parent
        then return $ nodeBlockChain parent
            ...
            maxChainM <-
                fmap (unValue <=< listToMaybe) $
                select $ from $ \t -> return $ max_ $ t ^. NodeBlockChain
...
getPivots = go []
  where
    go acc b
        ...
        | otherwise = do
            l <-
                fromMaybe (error "Houston, we have a problem") <$>
                getChainLowest b
            c <-
                fromMaybe (error "Ground Control to Major Tom") <$>
                getParentBlock l
            go (b : acc) c
...
getChainLowest
    ...
    => NodeBlock -> SqlPersistT m (Maybe NodeBlock)
getChainLowest nb =
    fmap (listToMaybe . map entityVal) $
    select $
    from $
    \t -> do
...
splitBlock
    :: MonadIO m
    => NodeBlock -> NodeBlock -> SqlPersistT m NodeBlock
splitBlock l r =
...
getBlockByHash
    :: MonadIO m
    => BlockHash -> SqlPersistT m (Maybe NodeBlock)
getBlockByHash h =
    fmap (listToMaybe . map entityVal) $
    select $
    from $
...
getBlocksByHash hashes = do
    nodes <-
        ...
        \t -> do
            ...
            return t
    return $ mapMaybe (\h -> find ((== shortHash h) . nodeBlockHash) nodes) hashes
...
getBlockByHeight block height = do
    ...
    fmap (listToMaybe . map entityVal) $
        ...
        from $
        \t -> do
            where_ $ chainPathQuery t forks &&. t ^. NodeBlockHeight ==. val height
            return t
...
getBlocksByHeight best heights = do
    ...
    nodes <-
        ...
        \t -> do
            where_ $
                chainPathQuery t forks &&. t ^. NodeBlockHeight `in_` valList heights
            return t
...
getBlockAfterTime
    :: MonadIO m
    => Timestamp -> SqlPersistT m (Maybe NodeBlock)
getBlockAfterTime ts = do
    n@NodeBlock {..} <- getBestBlock
    f genesisBlock n
  where
    f l r
        | nodeTimestamp r < ts = return Nothing
        | nodeTimestamp l >= ts = return $ Just l
        | (nodeBlockHeight r - nodeBlockHeight l) `div` 2 == 0 = return $ Just r
        | otherwise = do
            let rh = nodeBlockHeight r
                lh = nodeBlockHeight l
                mh = rh - (rh - lh) `div` 2
            m <-
                fromMaybe (error "My God, it’s full of stars!") <$>
                getBlockByHeight r mh
            if nodeTimestamp m > ts
                then f l m
                else f m r
...
getBlocksAtHeight
    :: MonadIO m
    => BlockHeight -> SqlPersistT m [NodeBlock]
...
evalNewChain
    :: MonadIO m
    => NodeBlock -> [NodeBlock] -> SqlPersistT m BlockChainAction
