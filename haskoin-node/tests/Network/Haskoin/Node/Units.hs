...
blockChainHead =
    ...
    do heads <- getHeads
       ...
       liftIO $
           assertEqual
               "Best block has correct hash"
               (nodeHash $ last chain3)
               (nodeHash bh)
       liftIO $
           assertEqual
               "Best block height is right"
               (nodeBlockHeight $ last chain3)
               (nodeBlockHeight bh)

forkNode :: App ()
forkNode =
    mockBlockChain >>
    do let l = last chain2
           r = last chain3
       bn <- splitBlock l r
       liftIO $ assertEqual "Split block are correct" (chain0 !! 1) bn
       commonLM <- getBlockByHeight l $ nodeBlockHeight bn
       when (isNothing commonLM) $
           liftIO $ assertFailure "Could not find fork on left side"
       let commonL = fromJust commonLM
       commonRM <- getBlockByHeight r $ nodeBlockHeight bn
       when (isNothing commonRM) $
           liftIO $ assertFailure "Could not find fork on right side"
       let commonR = fromJust commonRM
       firstLM <- getBlockByHeight l (nodeBlockHeight bn + 1)
       when (isNothing firstLM) $
           liftIO $ assertFailure "Could not find fork child on left side"
       let firstL = fromJust firstLM
       firstRM <- getBlockByHeight r (nodeBlockHeight bn + 1)
       when (isNothing firstLM) $
           liftIO $ assertFailure "Could not find fork child on right side"
       let firstR = fromJust firstRM
       liftIO $ assertEqual "Fork node is same in both sides" commonL commonR
       liftIO $
           assertEqual
               "Fork node connect with left side"
               (nodeHash commonL)
               (nodePrev firstL)
       liftIO $
           assertEqual
               "Fork node connect with right side"
               (nodeHash commonR)
               (nodePrev firstR)
       liftIO $ assertBool "After-fork chains diverge" $ firstL /= firstR
       liftIO $ assertEqual "Fork node matches hardcoded one" (chain0 !! 1) commonL
...
getSideChain =
    mockBlockChain >>
    do ch <- getBlocksFromHeight (chain2 !! 1) 0 0
       liftIO $ assertEqual "Side chain correct" sch ch
  where
    sch = genesisBlock : take 3 chain0 ++ take 2 chain1 ++ take 2 chain2

getNodesHeight :: App ()
getNodesHeight =
    mockBlockChain >>
    do ns <- getBlocksAtHeight 3
...
blockLocatorToHead = do
    ...
    ls <- blockLocator h
    liftIO $ assertEqual "Last is genesis" (last ls) (headerHash genesisHeader)
    liftIO $ assertEqual "First is current head" (head ls) (nodeHash h)
    last10 <-
        map nodeHash . reverse <$>
        getBlocksFromHeight h 0 (nodeBlockHeight h - 9)
    liftIO $ assertEqual "Last ten blocks contiguous" last10 (take 10 ls)
    let h10 = nodeBlockHeight h - 10
    bhs <-
        map (nodeHash . fromJust) <$>
        mapM
            (getBlockByHeight h)
            [h10, h10 - 2, h10 - 6, h10 - 14, h10 - 30, h10 - 62]
    liftIO $
        assertEqual
            "All block hashes correct"
...
blockLocatorToNode = do
    ...
    ls <- blockLocator n
    xs <- map nodeHash . reverse <$> getBlocksFromHeight n 0 0
...
splitNode = do
    ...
    liftIO $
        assertEqual "Left correct" ([chain0 !! 2] ++ take 2 chain1 ++ chain2) ls
    liftIO $ assertEqual "Right correct" chain3 rs

runUnit :: App () -> Assertion
runUnit action =
    runSqlite ":memory:" $
    do _ <- runMigrationSilent migrateHeaderTree
       action
...
manyBlocks b = tail $ reverse $ foldBlock (Just b) $ zip [18 .. 117] (repeat 4)

chain0 :: [NodeBlock]
chain0 = tail $ reverse $ foldBlock Nothing $ zip [1 .. 4] (repeat 0)
...
foldBlock nM = foldl f (maybeToList nM)
  where
    f [] _                = [genesisBlock]
